    ...
    <footer className="mt-auto border-t border-line bg-surface">
      <div className="mx-auto grid max-w-[1200px] gap-10 px-5 py-14 md:grid-cols-4 md:px-8">
        <div className="md:col-span-2">
          ...
          <p className="mt-3 max-w-[380px] font-body text-[13px] leading-relaxed text-ink-soft">
            {SITE.description}
          </p>
          <p className="mt-5 font-body text-[12px] text-muted">{FREE_SHIPPING_THRESHOLD_LABEL}</p>
        </div>

        {columns.map((col) => (
          <div key={col.title}>
            <p className="font-body text-[10px] uppercase tracking-[0.18em] text-muted">{col.title}</p>
            <ul className="mt-4 space-y-2.5">
              {col.links.map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="link-underline font-body text-[13px] text-ink-soft hover:text-ink"
                  >
                    {l.label}
                  </Link>
                </li>
      ...
      <div className="border-t border-line">
        <div className="mx-auto flex max-w-[1200px] flex-col gap-3 px-5 py-6 font-body text-[12px] text-muted md:flex-row md:items-center md:justify-between md:px-8">
          <p>
            {SITE.salon.addressLine}, {SITE.salon.city} ·{' '}
            <a href={telHref(SITE.salon.phone)} className="hover:text-ink">
              {SITE.salon.phone}
            </a>
          </p>
